...
function ProblemStepView({ problem }) {
    ...
    return <article className="problem-step-view">
        ...
        <div className="problem-step-view__steps">{problem.steps.map((step, index) => <section className="problem-step-view__step" key={step.id} aria-labelledby={`${step.id}-title`}><h4 id={`${step.id}-title`}>단계 {index + 1}</h4><p>{omitPeriodAfterTerminalBlank(step.segments).map((segment, segmentIndex) => segment.type === 'text' ? <span key={`${step.id}-text-${segmentIndex}`}><MathText inlineLatex>{segment.value}</MathText></span> : <span key={segment.id} className="problem-step-view__blank problem-step-view__blank--answer"><MathText inlineLatex>{segment.answer}</MathText></span>)}</p></section>)}</div>
