...
function AssessmentQuestionView({ problem, showAnswers, onScoreChange }) {
    ...
    return <article className="assessment-question" aria-labelledby={`assessment-question-${problem.id}`}>
        ...
        {problem.format === 'choice' && <ol className="assessment-question__choices">{problem.choices.map((choice, index) => { const isAnswer = String(index + 1) === problem.answer; return <li className={showAnswers && isAnswer ? 'assessment-question__choice--answer' : ''} key={choice}><span>{index + 1}</span><p>{choice}</p>{showAnswers && isAnswer && <strong>정답</strong>}</li>; })}</ol>}
        {problem.format === 'short' && <div className="assessment-question__short-answer"><span>학생 답안</span><div aria-hidden="true" />{showAnswers && <p><strong>정답</strong>{problem.answer}</p>}</div>}
        {problem.format === 'essay' && <div className="assessment-question__essay"><div className="assessment-question__essay-box"><span>학생 서술 답안</span></div>{showAnswers && <section className="assessment-question__model-answer"><h4>모범답안</h4><p>{problem.modelAnswer}</p></section>}<section className="assessment-question__rubric"><h4>채점 기준</h4><table><thead><tr><th>항목</th><th>부분 점수</th></tr></thead><tbody>{problem.rubric.map((item) => <tr key={item.label}><td>{item.label}</td><td>{item.score}점</td></tr>)}</tbody></table></section></div>}
    </article>;
}
