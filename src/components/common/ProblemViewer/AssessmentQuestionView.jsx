...
function AssessmentQuestionView({ problem, onScoreChange }) {
    ...
    return <article className="assessment-question" aria-labelledby={`assessment-question-${problem.id}`}>
        ...
        {problem.format === 'choice' && <ol className="assessment-question__choices">{problem.choices.map((choice, index) => { const isAnswer = String(index + 1) === problem.answer; return <li className={isAnswer ? 'assessment-question__choice--answer' : ''} key={choice}><span>{index + 1}</span><p>{choice}</p>{isAnswer && <strong>정답</strong>}</li>; })}</ol>}
        {problem.format === 'short' && <div className="assessment-question__short-answer"><span>학생 답안</span><div aria-hidden="true" /><p><strong>정답</strong>{problem.answer}</p></div>}
