...
function AssignmentProgressPanel({
    ...
    const worksheetId = assignment.analysisWorksheetId ?? assignment.worksheetId;
    const gradingPending = students.filter((student) => student.grading === 'pending').length;
            ...
            <div className="learning-panel__header learning-students__header">
                <div>
                    ...
                    {assignment.origin === 'custom' && (
                        <p>대상 {assignment.studentCount}명 · 배정 {formatDateTime(assignment.assignedAt)} · 마감 {formatDateTime(assignment.dueAt)}</p>
                    )}
                ...
                <table className="learning-students__table">
                    ...
                    <tbody>
                        {students.map((student) => {
                            ...
                            return (
                                <tr key={student.assignmentStudentId}>
                                    <td>{student.displayNumber}</td>
                                    <td>
                                        {student.name
                                            ? <Link className="learning-students__name" to={`/learning/weaknesses/students/${student.studentId}?worksheet=${worksheetId}`}>{student.name}</Link>
                                            : <span className="learning-students__muted">-</span>}
                                    </td>
                                    <td><span className={`learning-students__status learning-students__status--${student.status}`}>{statusLabels[student.status] ?? '미제출'}</span></td>
                                    <td>
                                    ...
                                    <td><span className="learning-students__muted">{formatDateTime(student.submittedAt, { includeYear: false })}</span></td>
                                </tr>
                            );
                        })}
