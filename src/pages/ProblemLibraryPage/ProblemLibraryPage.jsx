...
function ProblemLibraryPage() {
    ...
    const filtered = useMemo(() => worksheets.filter((worksheet) => {
        ...
        return (tab === 'all' || tab === kind) && (gradeId === 'all' || worksheet.gradeId === gradeId) && (term === 'all' || worksheet.term === term) && (status === 'all' || status === assignmentState) && worksheet.title.toLowerCase().includes(search.trim().toLowerCase());
