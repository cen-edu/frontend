        ...
        <article className="problem-step-view">
            ...
            <div className="problem-step-view__steps">
                {problem.steps.map((step, index) => (
                    <section className="problem-step-view__step" key={step.id} aria-labelledby={`${step.id}-title`}>
                        <h4 id={`${step.id}-title`}>단계 {index + 1}</h4>
                        <p>{step.segments.map((segment, segmentIndex) => segment.type === 'text'
                            ? <span key={`${step.id}-text-${segmentIndex}`}>{segment.value}</span>
                            : <span key={segment.id} className={`problem-step-view__blank${showAnswers ? ' problem-step-view__blank--answer' : ''}`}>{showAnswers ? segment.answer : <span className="problem-creation-sr-only">빈칸</span>}</span>)}</p>
                    </section>
                ))}
            </div>
