...
function CustomConfigTable({ configs, availableUnits, reason, onCountChange, onRemove, onAdd, onGenerate }) {
    ...
    return <section className="custom-config" aria-labelledby="custom-config-title">
        ...
        {configs.length === 0 ? <p className="custom-config__empty">{reason}</p> : <div className="custom-config__table-wrap"><table><thead><tr><th>취약 개념</th>{customStages.map((stage) => <th key={stage}>{customStageStepLabels[stage]}</th>)}<th><span className="custom-sr-only">제외</span></th></tr></thead><tbody>{configs.map((config) => <tr key={config.conceptId}>
            <td><strong>{config.conceptLabel}</strong><span>{config.manual ? '수동 추가 · 복습 제외' : `원본 ${config.sourceQuestionNo}번`}</span></td>
            {customStages.map((stage) => <td key={stage}><Stepper label={`${config.conceptLabel} ${customStageStepLabels[stage]}`} value={config.counts[stage]} disabled={config.manual && stage === 'retrace'} onChange={(value) => onCountChange(config.conceptId, stage, value)} /></td>)}
