...
function ProblemLibraryDetailPage() {
    ...
    if (worksheetQuery.isError || !worksheet) {
        const status = worksheetQuery.error?.status;
        const errorTitle = status === 401
            ? '로그인이 필요합니다.'
            : status === 403
                ? '학습지를 조회할 권한이 없습니다.'
                : status === 404
        ...
        return <section className="problem-library-detail problem-library-detail--missing"><h1>{errorTitle}</h1>{status !== 404 && <p>{worksheetQuery.error?.message}</p>}{![401, 403, 404].includes(status) && <button type="button" onClick={() => worksheetQuery.refetch()}>다시 시도</button>}<Link to="/problems/library">보관함으로 돌아가기</Link></section>;
    }
    if (!worksheet.problems.length) return <section className="problem-library-detail problem-library-detail--missing"><h1>학습지에 표시할 문항이 없습니다.</h1><Link to="/problems/library">보관함으로 돌아가기</Link></section>;
    ...
    const selectedSupport = selectedProblem.supportMode ?? (isCustom ? defaultSupportModes.custom : isAssessment ? defaultSupportModes.assessment : defaultSupportModes.practice);
    ...
    const previewControls = (
        <footer className="problem-library-detail__preview-controls">
            ...
            <span>{isAssessment ? '문항과 정답을 확인합니다.' : isCustom ? `${customStageLabels[selectedProblem.stage]} 단계 문제를 확인합니다.` : '문제의 풀이 과정과 개념 설명을 확인합니다.'}</span>
            ...
            <section className="problem-library-detail__section problem-library-detail__section--preview" aria-labelledby="preview-title">
                ...
                <div className="problem-library-detail__student-preview">
                    <div className="problem-library-detail__student-preview-content">
                        {isAssessment ? <div className="problem-library-detail__question-preview"><AssessmentQuestionView problem={selectedProblem} showRubric={false} />{previewControls}</div> : (
                            <PracticeProblemView
