...
function StudentAnalysisView({ worksheet, student, index, onMove }) {
    ...
    return <div className="student-analysis-view">
        ...
        <div className="student-analysis-view__metrics">
            {[['풀이 문항', metrics.solvedCount, '문항'], ['정답', metrics.correctCount, '문항'], ['정답률', metrics.scoreRate, '%'], ['힌트 사용', metrics.hints, '회']].map(([label, value, unit]) => <div key={label}><span>{label}</span><strong>{value}<small>{unit}</small></strong></div>)}
        </div>
        ...
        <div className="student-analysis-view__insight"><strong>{weakConcept || '응답 패턴'}</strong>에서 반복 오류가 관찰되었습니다. 독립 정답률은 {metrics.independentRate}%입니다.</div>
        <section className="student-priority">
            ...
            <dl><div><dt>상태</dt><dd>{statusLabels[student.status]}</dd></div><div><dt>누적 오답</dt><dd>{student.responses.filter((response) => response.score < response.maxScore && response.gradedBy !== null).length}회</dd></div><div><dt>최근 연속 독립 정답</dt><dd>{recentIndependent < 0 ? student.responses.length : recentIndependent}회</dd></div></dl>
        </section>
        ...
        <section className="student-analysis-view__results diagnosis-card">
            <div className="student-analysis-view__results-heading"><div><span>지도 근거 확인</span><h2>문항별 결과</h2></div><div className="diagnosis-tabs">{[['all', '전체'], ['wrong', '오답'], ['hint', '힌트 후 정답'], ['pending', '채점 대기']].map(([value, label]) => <button type="button" key={value} className={resultFilter === value ? 'diagnosis-tabs__button diagnosis-tabs__button--active' : 'diagnosis-tabs__button'} onClick={() => setResultFilter(value)}>{label}</button>)}</div></div>
        ...
        <section className="diagnosis-card learning-record">
            ...
            <div className="learning-record__wrap"><table><thead><tr><th>문항</th><th>영역</th><th>결과</th><th>힌트</th><th>학급 정답률</th></tr></thead><tbody>{worksheet.questions.map((question) => { const response = student.responses.find((item) => item.no === question.no); const accuracy = getQuestionAccuracy(worksheet, question.no); const label = response.gradedBy === null ? '채점 대기' : response.score === response.maxScore ? response.hintUsed ? '힌트 후 정답' : '독립 정답' : '오답'; return <tr key={question.no}><td>{question.no}번</td><td>{areaLabels[question.area]}</td><td>{label}</td><td>{response.hintUsed ? '사용' : '—'}</td><td>{accuracy.rate}%</td></tr>; })}</tbody></table></div>
            <div className="learning-record__observation"><strong>관찰 요약</strong><p>{weakConcept || '현재 응답'} 관련 문항에서 정답의 근거를 말로 설명하게 하고, 힌트 없이 같은 구조를 다시 해결하는지 확인해 주세요.</p></div>
        </section>
    </div>;
}
