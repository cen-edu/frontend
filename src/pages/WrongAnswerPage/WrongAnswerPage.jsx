...
function WrongAnswerPage() {
    ...
    const filteredWorksheets = useMemo(() => worksheets.filter((worksheet) => (classId === 'all' || worksheet.classId === classId) && (period === 'all' || worksheet.period === period) && (status === 'all' || worksheet.assignStatus === status) && (!search.trim() || worksheet.title.toLowerCase().includes(search.trim().toLowerCase()))), [classId, period, search, status, worksheets]);
    ...
    useEffect(() => { if (previousWorksheetId.current !== selectedId) { setSelectedItemIds([]); setSelectedStudentIds([]); setRemindedIds([]); setPanelItemId(null); previousWorksheetId.current = selectedId; } }, [selectedId]);
    ...
    return (
        <section className="wrong-answer-page" aria-label="오답 학습">
            <div className="wrong-answer-page__toolbar"><div className="wrong-answer-page__filters"><CustomSelect label="반 선택" value={classId} options={wrongAnswerFilterOptions.classes} onChange={setClassId} width={174} /><CustomSelect label="기간 선택" value={period} options={wrongAnswerFilterOptions.periods} onChange={setPeriod} width={116} /><div className="wrong-answer-page__tabs" role="group" aria-label="오답 학습 상태">{statusTabs.map((tab) => <button key={tab.value} type="button" className={`wrong-answer-page__tab${status === tab.value ? ' wrong-answer-page__tab--active' : ''}`} aria-pressed={status === tab.value} onClick={() => setStatus(tab.value)}>{tab.label}</button>)}</div></div><label className="wrong-answer-page__search"><span className="wrong-answer-sr-only">학습명 검색</span><input type="search" placeholder="학습명 검색" value={search} onChange={(event) => setSearch(event.target.value)} /><i className="bi bi-search" aria-hidden="true" /></label></div>
            <div className="wrong-answer-page__content"><WrongAnswerWorksheetList worksheets={filteredWorksheets} selectedId={selectedId} onSelect={setSelectedId} />
                {worksheet ? <main className="wrong-answer-detail"><header className="wrong-answer-detail__header"><div><span>{worksheet.className} · {worksheet.type === 'assessment' ? '종합평가' : '일반 학습'}</span><h2>{worksheet.title}</h2></div><span className="wrong-answer-detail__guide"><i className="bi bi-info-circle" aria-hidden="true" /> 해설 검토가 필요한 항목은 배정할 수 없어요.</span></header><WrongQuestionList worksheet={worksheet} selectedIds={selectedItemIds} onToggle={toggleItem} onSelectAll={toggleAllItems} onOpen={(item) => setPanelItemId(item.id)} onAssign={() => setAssignOpen(true)} /><ReviewProgressTable worksheet={worksheet} selectedIds={selectedStudentIds} remindedIds={remindedIds} onToggle={toggleStudent} onToggleAll={toggleAllStudents} onRemind={() => setRemindedIds((current) => [...new Set([...current, ...selectedStudentIds])])} /></main> : <div className="wrong-answer-detail wrong-answer-detail--empty"><i className="bi bi-search" aria-hidden="true" /><p>조건에 맞는 학습이 없습니다.</p></div>}
            </div>
