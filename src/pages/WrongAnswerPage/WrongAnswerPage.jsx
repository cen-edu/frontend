    ...
    return (
        <section className="wrong-answer-page" aria-labelledby="wrong-answer-page-title">
            ...
            <div className="wrong-answer-page__content"><WrongAnswerWorksheetList worksheets={filteredWorksheets} selectedId={selectedId} onSelect={setSelectedId} />
                {worksheet ? <main className="wrong-answer-detail"><header className="wrong-answer-detail__header"><div><span>{worksheet.className} · {worksheet.type === 'assessment' ? '종합평가' : '일반 학습'}</span><h2>{worksheet.title}</h2></div><span className="wrong-answer-detail__guide"><i className="bi bi-info-circle" aria-hidden="true" /> 해설 검토가 필요한 항목은 배정할 수 없습니다.</span></header><WrongQuestionList worksheet={worksheet} selectedIds={selectedItemIds} onToggle={toggleItem} onSelectAll={toggleAllItems} onOpen={(item) => setPanelItemId(item.id)} onAssign={() => setAssignOpen(true)} /><ReviewProgressTable worksheet={worksheet} selectedIds={selectedStudentIds} remindedIds={remindedIds} onToggle={toggleStudent} onToggleAll={toggleAllStudents} onRemind={() => setRemindedIds((current) => [...new Set([...current, ...selectedStudentIds])])} /></main> : <div className="wrong-answer-detail wrong-answer-detail--empty"><i className="bi bi-search" aria-hidden="true" /><p>조건에 맞는 학습이 없습니다.</p></div>}
            </div>
