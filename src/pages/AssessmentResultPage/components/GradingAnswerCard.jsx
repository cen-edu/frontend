    ...
    return (
        <article className={`grading-answer-card${isAuto ? ' grading-answer-card--auto' : ''}`}>
            <div className="grading-answer-card__header">
                <div><span>{question.no}번</span><strong>{formatLabels[question.format]}</strong></div>
                {isPractice && practiceResult && <span
                    className={`grading-answer-card__practice-current grading-answer-card__practice-current--${practiceResult}`}>{practiceResults.find((item) => item.value === practiceResult)?.description}</span>}
                {!isPractice && typeof answer?.score === 'number' &&
                    <span className="grading-answer-card__result">{answer.score}/{question.maxScore} <i
                        className="bi bi-check-circle-fill" aria-hidden="true"/></span>}
            </div>
            <p className="grading-answer-card__prompt">{question.prompt}</p>
            {!isChoiceReview && !isUnsubmitted && answer?.answerImage && <figure className="grading-answer-card__scan">
                <figcaption><i className="bi bi-pencil" aria-hidden="true"/> {student.name}의 원본 답안</figcaption>
                <img src={answer.answerImage} alt={`${student.name} 학생이 필기로 작성한 ${question.no}번 원본 답안`}/>
            </figure>}
            {!isChoiceReview && <div
                className={`grading-answer-card__response${isShortAutoWrong ? ' grading-answer-card__response--wrong' : ''}`}>
                <span>{student.name}의 답</span><p
                className={isUnsubmitted ? 'grading-answer-card__answer grading-answer-card__answer--empty' : 'grading-answer-card__answer'}>{isUnsubmitted ? '미제출' : studentAnswer}</p>
            </div>}
            {isChoiceReview && isUnsubmitted &&
                <p className="grading-answer-card__choice-empty"><i className="bi bi-dash-circle"
                                                                    aria-hidden="true"/> 미제출 · 선택한 보기가 없습니다</p>}
            {isChoiceReview && <ol className="grading-answer-card__choices" aria-label={`${question.no}번 객관식 보기`}>
                {question.choices.map((choice, index) => {
                    ...
                    return <li key={`${index}-${choice}`}
                               className={`grading-answer-card__choice${isSelected ? ' grading-answer-card__choice--selected' : ''}${state}`}>
            ...
            {isPractice && <div className="grading-answer-card__grading grading-answer-card__grading--practice">
                ...
                <div className="grading-answer-card__practice-results" role="group"
                     aria-label={`${student.name} ${question.no}번 채점 결과`}>
                    {practiceResults.map((result) => <button key={result.value} type="button"
                                                             className={`grading-answer-card__practice-result grading-answer-card__practice-result--${result.value}${practiceResult === result.value ? ' grading-answer-card__practice-result--selected' : ''}`}
                                                             disabled={isUnsubmitted} aria-label={result.description}
                                                             aria-pressed={practiceResult === result.value}
                                                             onClick={() => onPracticeResult(result.value)}>
                        <strong>{result.label}</strong><span>{result.description}</span></button>)}
                </div>
            </div>}
                ...
                <div className="grading-answer-card__rubric grading-answer-card__rubric--result">
                    {question.rubric.map((item, index) => {
                        ...
                        return <div key={item.label}
                                    className={`grading-answer-card__rubric-result${satisfied ? ' grading-answer-card__rubric-result--satisfied' : ''}`}>
                            <div className="grading-answer-card__rubric-summary">
                                <i className={`bi ${satisfied ? 'bi-check-circle-fill' : 'bi-dash-circle'}`}
                                   aria-hidden="true"/>
            ...
            {!isPractice && <div className="grading-answer-card__grading">
                ...
                <div className="grading-answer-card__scores" aria-label={`${student.name} ${question.no}번 점수 선택`}>
                    {scoreOptions.map((score) => <button key={score} type="button"
                                                         className={answer?.score === score ? 'grading-answer-card__score grading-answer-card__score--selected' : 'grading-answer-card__score'}
                                                         disabled={isUnsubmitted}
                                                         onClick={() => onScore(score)}>{score}점</button>)}
                </div>
                {question.format !== 'short' && !!question.rubric.length &&
                    <div className="grading-answer-card__rubric">
                        {question.rubric.map((item, index) => <div key={item.label}
                                                                   className="grading-answer-card__rubric-row">
                            <CustomCheckbox label={`${item.label} ${item.score}점`} checked={rubricChecks[index]}
                                            disabled={isUnsubmitted}
                                            onChange={() => onRubric(index)}/><span>{item.label}</span><strong>{item.score}점</strong>
                        </div>)}
                    </div>}
            </div>}
        </article>
