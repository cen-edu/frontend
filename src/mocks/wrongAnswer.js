...
const rawWrongAnswerWorksheets = [
    {
        ...
        wrongItems: [
            ...
            { id: 'q7', no: 7, conceptId: 'divisor', conceptLabel: '약수의 개수', prompt: '72의 약수의 개수를 구하세요.', finalAnswer: '12개', solutionText: '', solutionReviewed: false, wrongStudentIds: [102, 103, 106, 107], commonWrongInputs: [{ input: '10개', count: 2 }] },
            { id: 'q3', no: 3, conceptId: 'prime', conceptLabel: '소인수분해', prompt: '60을 소인수분해하세요.', finalAnswer: '2²×3×5', solutionText: '60을 소수로 차례로 나누면 60=2×2×3×5이므로 2²×3×5입니다.', solutionReviewed: true, wrongStudentIds: [103, 107, 108], commonWrongInputs: [{ input: '2×30', count: 2 }] },
        ],
        assignments: [],
    },
    {
        ...
        wrongItems: [
            { id: 'c-common', conceptId: 'common', conceptLabel: '공통 소인수 선택', prompt: '두 수의 공통인 소인수를 고르는 학습입니다.', finalAnswer: '두 수 모두의 인수인 소수', solutionText: '각 수를 소인수분해한 뒤 두 식에 공통으로 들어 있는 소인수를 찾습니다.', solutionReviewed: true, wrongStudentIds: [101, 103, 105, 108], commonWrongInputs: [{ input: '2×6', count: 3 }] },
            { id: 'c-prime', conceptId: 'prime', conceptLabel: '소인수분해', prompt: '합성수를 소수의 곱으로 나타내는 학습입니다.', finalAnswer: '소수만 남을 때까지 나누기', solutionText: '가장 작은 소수부터 차례로 나누고 같은 소수는 지수로 정리합니다.', solutionReviewed: true, wrongStudentIds: [103, 107, 108], commonWrongInputs: [{ input: '2×30', count: 2 }] },
            { id: 'c-exponent', conceptId: 'exponent', conceptLabel: '지수 비교', prompt: '최대공약수와 최소공배수를 구할 때 지수를 비교합니다.', finalAnswer: '최대공약수는 작은 지수, 최소공배수는 큰 지수', solutionText: '', solutionReviewed: false, wrongStudentIds: [102, 105, 107], commonWrongInputs: [{ input: '항상 큰 지수', count: 2 }] },
        ],
        assignments: [
    ...
    },
    {
        ...
        wrongItems: [
            { id: 'custom-common', conceptId: 'common', conceptLabel: '공통 소인수 적용', prompt: '서로 다른 구조에서 공통 소인수를 찾는 학습입니다.', finalAnswer: '공통인 소수와 지수 확인', solutionText: '수의 크기보다 소인수분해한 식에서 공통 요소를 먼저 확인합니다.', solutionReviewed: true, wrongStudentIds: [102, 103, 108], commonWrongInputs: [{ input: '공배수 선택', count: 2 }] },
        ],
        assignments: [
...
const solutionStep = (item) => ({
    id: `${item.id}-solution-1`,
    conceptId: item.conceptId,
    segments: [
        ...(item.solutionText ? [{ type: 'text', value: item.solutionText }] : []),
        { type: 'blank', id: `${item.id}-answer`, answer: item.finalAnswer },
    ],
});

export const wrongAnswerWorksheets = rawWrongAnswerWorksheets.map((worksheet) => ({
    ...worksheet,
    wrongItems: worksheet.wrongItems.map(({ finalAnswer, solutionText, solutionReviewed, ...item }) => ({
        ...item,
        steps: [solutionStep({ ...item, finalAnswer, solutionText })],
        teacherNote: '',
        solutionStatus: worksheet.id === 'factor-assessment' && !solutionReviewed ? 'needs-review' : worksheet.id === 'factor-assessment' ? 'reviewed' : 'embedded',
    })),
}));

export const isWrongItemAssignable = (item) => item.solutionStatus !== 'needs-review';
