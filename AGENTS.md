...
- 맞춤 문제 생성 화면의 재출제 제안은 원본 배정 ID와 학생 ID로 `/assignments/{assignmentId}/students/{studentId}/reissue-proposal`을 조회한다. 응답의 `review | similar | advanced` 권장 수와 `maxCount`를 각각 복습·유사·응용 단계에 매핑하되, 세 단계의 `proposedCount`가 모두 0인 소분류는 취약점이 없는 개념으로 보고 문항 구성 제안·제안 근거·제안 개수에서 제외한다. 필터링 후 빈 소분류 배열은 정상적인 제안 없음 상태로 처리한다. 맞춤 생성·학생 단위 배포 API가 제공되기 전에는 목 데이터로 생성·배포를 대신 실행하지 않는다.
