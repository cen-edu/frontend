...
- 맞춤 문제 생성은 최신 재출제 제안 범위 안에서 `/api/teacher/custom-problems/generate/async`에 `clientRequestId`, 원본 `sourceAssignmentId`, `studentId`, 소분류별 복습·유사·응용 수량을 보내고 `/api/teacher/problems/generation-jobs/{jobId}`를 종료 상태까지 폴링한다. 네트워크 오류로 접수 요청을 다시 보낼 때는 같은 `clientRequestId`를 재사용하고 새로운 생성에는 새 UUID를 사용한다. 결과는 `slotIndex`로 정렬하고 `READY` 슬롯만 미리보기에 표시하며 `PARTIALLY_FAILED`는 성공 문항과 실패 수를 함께 안내한다. 학생 단위 저장·배포 API가 제공되기 전에는 생성 결과를 목 데이터로 저장하거나 배포하지 않는다.
