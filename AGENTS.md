...
- 맞춤 문제 생성 화면의 재출제 제안은 원본 배정 ID와 학생 ID로 `/assignments/{assignmentId}/students/{studentId}/reissue-proposal`을 조회한다. 응답의 `review | similar | advanced` 권장 수와 `maxCount`를 각각 복습·유사·응용 단계에 매핑하되, 세 단계의 `proposedCount`가 모두 0인 소분류는 취약점이 없는 개념으로 보고 문항 구성 제안·제안 근거·제안 개수에서 제외한다. 필터링 후 빈 소분류 배열은 정상적인 제안 없음 상태로 처리한다.
- 맞춤 문제 생성은 최신 재출제 제안 범위 안에서 `/api/teacher/custom-problems/generate/async`에 `clientRequestId`, 원본 `sourceAssignmentId`, `studentId`, 소분류별 복습·유사·응용 수량을 보내고 `/api/teacher/problems/generation-jobs/{jobId}`를 종료 상태까지 폴링한다. 네트워크 오류로 접수 요청을 다시 보낼 때는 같은 `clientRequestId`를 재사용하고 새로운 생성에는 새 UUID를 사용한다. 결과는 `slotIndex`로 정렬하고 `READY` 슬롯만 미리보기에 표시하며 `PARTIALLY_FAILED`는 성공 문항과 실패 수를 함께 안내한다. 생성 결과는 `origin: 'custom'`, 원본 `sourceAssignmentId`, 직전 회차의 `parentWorksheetId`, `sessionId`, `customStage`를 포함해 `/api/teacher/worksheets`에 저장하고, 응답의 `worksheetId`를 `/api/teacher/worksheets/{worksheetId}/assignments`에 `classId: null`, 생성 대상 `studentId`, `dueAt`과 함께 보내 개인 배포한다. 첫 회차의 부모는 원본 배정의 학습지, 다음 회차부터는 해당 학생의 직전 맞춤 배정 학습지로 연결한다.
...
- 취약점 분석 PDF는 학급 `/assignments/{assignmentId}/report.pdf`, 학생 `/assignments/{assignmentId}/students/{studentId}/report.pdf`를 공통 `httpClient`로 `blob` 조회한다. `Accept: application/pdf`를 보내고 응답 `Content-Type`을 확인하며, 파일명은 CORS의 `Content-Disposition` 노출에 의존하지 않고 각각 `class-analysis-report-{assignmentId}.pdf`, `analysis-report-{assignmentId}-{studentId}.pdf`로 클라이언트에서 만든다. PDF 실패 응답의 JSON Blob은 공통 `ApiError`로 변환한다.
...
- AI 문제 편집은 `/api/teacher/problems/authoring-sessions/{sessionId}`의 `/edit/turns`, `/status`, `/preview`를 사용한다. 화면 선택 영역은 스냅샷의 `stepKey`, `choiceKey`, `answerUnitKey`, `rubricKey` 같은 논리 키와 API `targetType`으로 변환하며 `semanticPatch.operations`를 클라이언트에서 직접 적용하지 않는다. `REQUEST_CONFIRMATION` 뒤에는 이전 사용자·AI 대화를 `history`에 포함한 확인 턴을 보내고, `CONFIRM_EXECUTION`이면 상태를 폴링해 `IDLE`과 `readyForFinalization: true`를 확인한 뒤 최신 preview로 해당 `sessionId` 문항만 교체한다.
